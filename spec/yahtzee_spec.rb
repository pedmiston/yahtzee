...
describe "Yahtzee" do
  ...
  it "should not score a full house with more than two unique values" do
    dice = [1, 1, 2, 2, 4]
    result = @yahtzee.score_by_rule(dice, "full_house")
    expect(result).to eq(0)
  end

  it "should not score a full house that doesn't contain a pair and a three of a kind" do
    dice = [1, 1, 2, 1, 1]
    result = @yahtzee.score_by_rule(dice, "full_house")
    expect(result).to eq(0)
  end
  ...
  it "should not score a false small straight" do
    dice = [6, 5, 4, 3, 3]
    result = @yahtzee.score_by_rule(dice, "small_straight")
    expect(result).to eq(0)
  end

  it "should score a large straight" do
    dice = [6, 4, 5, 3, 2]
    result = @yahtzee.score_by_rule(dice, "large_straight")
    expect(result).to eq(20)
  end

  it "should not score a false large straight" do
    dice = [2, 2, 3, 4, 5]
    result = @yahtzee.score_by_rule(dice, "large_straight")
    expect(result).to eq(0)
  end
